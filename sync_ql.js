...
async function getToken() {
    ...
    await httpRequest('get', request, printCaller())
...
async function add(name, value, remarks) {
    let url = `${ql_host}/open/envs`
    let body = JSON.stringify([{ value, name, remarks }])
    let requestObject = getReqObject(url, body)
    await httpRequest('post', requestObject, printCaller())
    let result = httpResult;
    if (result.code == 200) {
        $.log(`📢 成功添加青龙环境变量: ${name}`)
    } else {
...
async function update(name, value, remarks, id) {
    let url = `${ql_host}/open/envs`
    let body = JSON.stringify({ value, name, remarks, id })
    let requestObject = getReqObject(url, body)
    await httpRequest('put', requestObject, printCaller())
    let result = httpResult;
    if (result.code == 200) {
        $.log(`📢 成功更新青龙环境变量: ${name}`)
    } else {
...
function getReqObject(url, body) {
    let request = {
        url: url,
        headers: {
...
async function httpRequest(method, request, caller) {
    httpResult = null
    return new Promise((resolve) => {
        $[method](request, async (err, resp, data) => {
            try {
                if (err) {
                    $.log(`${caller}: ${method}请求失败`);
...
function Env(name, opts) {
    ...
    return new (class {
        ...
        put(opts, callback = () => { }) {
            const method = opts.method ? opts.method.toLocaleLowerCase() : 'put'
            // 如果指定了请求体, 但没指定`Content-Type`, 则自动生成
            if (opts.body && opts.headers && !opts.headers['Content-Type']) {
                opts.headers['Content-Type'] = 'application/x-www-form-urlencoded'
            }
